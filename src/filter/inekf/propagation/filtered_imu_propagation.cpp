...
FilteredImuPropagation::FilteredImuPropagation(
    ...
    : Propagation::Propagation(imu_data_buffer_mutex_ptr),
      imu_data_buffer_ptr_(imu_data_buffer_ptr),
      angular_velocity_data_buffer_ptr_(angular_velocity_data_buffer_ptr),
      angular_velocity_data_buffer_mutex_ptr_(
          angular_velocity_data_buffer_mutex_ptr),
      error_type_(error_type),
      enable_imu_bias_update_(enable_imu_bias_update) {
      propagation_type_ = PropagationType::FILTERED_IMU;
      ...
      Eigen::Quaternion<double> quarternion_imu2body(
          quat_imu2body[0], quat_imu2body[1], quat_imu2body[2],
          quat_imu2body[3]);
      ...
      double gyro_std = config_["noises"]["gyroscope_std"]
                            ...
                            : 0.1;
      double accel_std
          = config_["noises"]["accelerometer_std"]
      ...
      gyro_bias_cov_ = gyro_bias_std * gyro_bias_std
                       * Eigen::Matrix<double, 3, 3>::Identity();
      accel_cov_
          = accel_std * accel_std * Eigen::Matrix<double, 3, 3>::Identity();
    ...
    Eigen::MatrixXd FilteredImuPropagation::StateTransitionMatrix(
      ...
      // Compute the complicated bias terms (derived for the left invariant
      // case)
      ...
      Eigen::Matrix3d Phi35L
          = G0t
            * (ax * G3t * dt3
               - ((thetadtsinthetadt + 2 * costhetadt - 2) / (theta4))
                     * (wx * ax)
               ...
               - ((2 * thetadt2 + 8 * thetadtsinthetadt + 16 * costhetadt
                  ...
                  / (8 * theta6))
                     * (wx * ax * wx2)
               + ((thetadt3 + 6 * thetadt - 12 * sinthetadt
                   + 6 * thetadtcosthetadt)
                  / (6 * theta5))
                     * (wx2 * ax)
               - ((6 * thetadt2 + 16 * costhetadt - cos2thetadt - 15)
                  / (8 * theta6))
                     * (wx2 * ax * wx)
               + ((4 * thetadt3 + 6 * thetadt - 24 * sinthetadt
                   - 3 * sin2thetadt + 24 * thetadtcosthetadt)
                  / (24 * theta7))
                     * (wx2 * ax * wx2));
    ...
    Eigen::MatrixXd FilteredImuPropagation::DiscreteNoiseMatrix(
      ...
      Eigen::MatrixXd Qc
          = state.get_continuous_noise_covariance();    // Landmark noise terms
                                                        // will remain zero
    ...
    void FilteredImuPropagation::InitImuBias() {
      ...
      if (bias_init_vec_.size() < init_bias_size_) {
        ...
        const ImuMeasurementPtr imu_measurement = imu_data_buffer_ptr_->front();
        imu_data_buffer_ptr_->pop();
        sensor_data_buffer_mutex_ptr_.get()->unlock();

        Eigen::Vector3d w
            = imu_measurement->get_ang_vel();    // Angular Velocity
        Eigen::Vector3d a
        ...
        std::cout << "IMU bias initialized to: " << avg.transpose()
                  << std::endl;
        bg0_ = avg.head<3>();
        ba0_ = avg.tail<3>();
        bias_initialized_ = true;
    ...
    const Eigen::Vector3d FilteredImuPropagation::get_estimate_gyro_bias()
        const {
      return bg0_;
    }

    const Eigen::Vector3d FilteredImuPropagation::get_estimate_accel_bias()
        const {
      return ba0_;
    }

    const bool FilteredImuPropagation::get_bias_initialized() const {
