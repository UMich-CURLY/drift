...
const IMUQueuePtr FilteredImuPropagation::get_sensor_data_buffer_ptr() const {
...
const Eigen::Vector3d FilteredImuPropagation::get_estimate_gyro_bias() const {
  return bg0_;
}

const Eigen::Vector3d FilteredImuPropagation::get_estimate_accel_bias() const {
  return ba0_;
}
...
bool FilteredImuPropagation::set_initial_state(RobotState& state) {
  ...
  if (use_imu_ori_est_to_init_) {
    Eigen::Quaternion<double> quat = imu_measurement->get_quaternion();
    R0 = quat.toRotationMatrix();    // Initialize based on VectorNav
                                     // estimate
    std::cout << "R0: \n" << R0 << std::endl;
  }
  Eigen::Vector3d p0
