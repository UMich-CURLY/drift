...
Eigen::Matrix3d Gamma_SO3(const Eigen::Vector3d& w, int m) {
  ...
  if (theta < TOLERANCE) {
    return (1.0 / factorial(m))
           * I;    // TODO: There is a better small value approximation for
                   // exp() given in Trawny p.19
  }
  ...
  switch (m) {
    ...
    case 1:    // Left Jacobian of SO(3)
      ...
      return I + ((1 - cos(theta)) / theta2) * A
             + ((theta - sin(theta)) / (theta2 * theta)) * A * A;
    ...
    default:    // General case
      Eigen::Matrix3d R
          = I + (sin(theta) / theta) * A + ((1 - cos(theta)) / theta2) * A * A;
      Eigen::Matrix3d S = I;
      Eigen::Matrix3d Ak = I;
      ...
      if (m == 0) {
        ...
        return (1.0 / kfactorial) * I
               + (pow(-1, (m + 1) / 2) / pow(theta, m + 1)) * A * (R - S);
      } else {    // even
        return (1.0 / kfactorial) * I
               + (pow(-1, m / 2) / pow(theta, m)) * (R - S);
      }
  }
}
