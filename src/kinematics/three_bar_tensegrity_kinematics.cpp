...
ThreeBarTensegrityKinematics::ThreeBarTensegrityKinematics() {
...
void ThreeBarTensegrityKinematics::ComputeKinematics() {
...
const Eigen::Vector3d ThreeBarTensegrityKinematics::get_init_velocity(
    const Eigen::Vector3d& w) {
  Eigen::Vector3d velocity = Eigen::Vector3d::Zero();

  // if (this->get_contact(FR) == 1) {
  //   Eigen::Vector3d pRF = p_Body_to_FrontRightFoot(encoders_);    //
  //   {I}_p_{IRF} Eigen::Matrix<double, 3, 12> J_pRF =
  //   Jp_Body_to_FrontRightFoot(encoders_); velocity = -J_pRF * d_encoders_ -
  //   lie_group::skew(w) * pRF;    // {I}_v_{WI}
  // } else if (this->get_contact(FL) == 1) {
  //   Eigen::Vector3d pLF = p_Body_to_FrontLeftFoot(encoders_);    //
  //   {I}_p_{ILF} Eigen::Matrix<double, 3, 12> J_pLF =
  //   Jp_Body_to_FrontLeftFoot(encoders_); velocity = -J_pLF * d_encoders_ -
  //   lie_group::skew(w) * pLF;    // {I}_v_{WI}
  // } else if (this->get_contact(HR) == 1) {
  //   Eigen::Vector3d pRH = p_Body_to_HindRightFoot(encoders_);    //
  //   {I}_p_{IRH} Eigen::Matrix<double, 3, 12> J_pRH =
  //   Jp_Body_to_HindRightFoot(encoders_); velocity = -J_pRH * d_encoders_ -
  //   lie_group::skew(w) * pRH;    // {I}_v_{WI}
  // } else if (this->get_contact(HL) == 1) {
  //   Eigen::Vector3d pLH = p_Body_to_HindLeftFoot(encoders_);    //
  //   {I}_p_{ILH} Eigen::Matrix<double, 3, 12> J_pLH =
  //   Jp_Body_to_HindLeftFoot(encoders_); velocity = -J_pLH * d_encoders_ -
  //   lie_group::skew(w) * pLH;    // {I}_v_{WI}
  // }
  return velocity;
}

int ThreeBarTensegrityKinematics::get_num_legs() { return NLEG; }
}    // namespace measurement::kinematics