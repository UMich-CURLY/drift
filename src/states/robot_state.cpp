...
RobotState::RobotState(se_k_3::SEK3& X) : 
    X_(X.get_X()), Theta_(Eigen::MatrixXd::Zero(3*(this->dimX()-3),1)) {
    P_ = Eigen::MatrixXd::Identity(3*this->dimX()+this->dimTheta()-6, 3*this->dimX()+this->dimTheta()-6);
}

// Initialize with SEK3 and Theta
RobotState::RobotState(se_k_3::SEK3& X, const Eigen::VectorXd& Theta) : 
    X_(X.get_X()), Theta_(Theta) {
    P_ = Eigen::MatrixXd::Identity(3*this->dimX()+this->dimTheta()-6, 3*this->dimX()+this->dimTheta()-6);
}

// Initialize with SEK3, Theta and P
RobotState::RobotState(se_k_3::SEK3& X, const Eigen::VectorXd& Theta, const Eigen::MatrixXd& P) : 
...
int RobotState::add_aug_state(int idx_map, const Eigen::Vector3d& aug) {
    ...
    Eigen::MatrixXd X_aug = Eigen::MatrixXd::Identity(this->dimX()+1, this->dimX()+1);
...
void RobotState::set_aug_state(int idx_map, int idx_state, const Eigen::Vector3d& aug) {
...
void RobotState::del_aug_state(int idx_map, int idx_state) {
    ...
    Eigen::MatrixXd X_aug = Eigen::MatrixXd::Identity(this->dimX()-1, this->dimX()-1);
    X_aug.block(0,0, idx_maps_[idx_map][idx_state], idx_maps_[idx_map][idx_state]) = X_.block(0,0, idx_maps_[idx_map][idx_state], idx_maps_[idx_map][idx_state]);
    X_aug.block(idx_maps_[idx_map][idx_state], idx_maps_[idx_map][idx_state],
        this->dimX()-idx_maps_[idx_map][idx_state]-1, this->dimX()-idx_maps_[idx_map][idx_state]-1) 
        = X_.block(idx_maps_[idx_map][idx_state]+1, idx_maps_[idx_map][idx_state]+1, this->dimX()-idx_maps_[idx_map][idx_state]-1, this->dimX()-idx_maps_[idx_map][idx_state]-1);
    X_ = X_aug;
...
void RobotState::set_aug_bias(int idx_map, int idx_state, const Eigen::Vector3d& baug) {
...
void RobotState::del_aug_bias(int idx_map, int idx_state) {
    ...
    Theta_aug.block(0,idx_bias,1,dim-idx_bias-3) = Theta_.block(0,idx_bias+3,1,dim-idx_bias-3);
    Theta_ = Theta_aug;
    idx_maps_[idx_map].erase(idx_state);
}
...
void RobotState::set_aug_cov(int idx_map, int idx_state, const Eigen::Matrix3d& cov) {
...
void RobotState::del_aug_cov(int idx_map, int idx_state) {
    ...
    P_aug.block(idx_cov, idx_cov, dim-idx_cov-3, dim-idx_cov-3) = P_.block(idx_cov+3, idx_cov+3,dim-idx_cov-3,dim-idx_cov-3);
    P_ = P_aug;
    idx_maps_[idx_map].erase(idx_state);
}
