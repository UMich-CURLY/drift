...
Eigen::MatrixXd SEK3::get_aug(std::string key) {
  return X_.block(0, map_aug_[key] - 1, 3, 1);
}
...
void SEK3::del_aug(std::string key) {
  ...
  if (idx < numRows)
    matrix.block(idx, 0, numRows - idx, numCols)
        = matrix.block(idx + 1, 0, numRows - idx, numCols);
  matrix.conservativeResize(numRows, numCols);
  numCols--;
  if (idx < numCols)
    matrix.block(0, idx, numRows, numCols - idx)
        = matrix.block(0, idx + 1, numRows, numCols - idx);
  matrix.conservativeResize(numRows, numCols);
  X_ = matrix;
  map_aug_.erase(key);
  K_ = numRows;
