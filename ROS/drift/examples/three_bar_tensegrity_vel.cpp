...
/**
 *  @file   tensegrity.cpp
 *  @author Tzu-Yuan Lin, Wenzhe Tong
 *  @brief  Test file for three-bar tensegrity robot (IMU propagation + Legged
...
int main(int argc, char** argv) {
  ...
  std::string config_file
      = project_dir + "/ROS/drift/config/tensegrity/ros_comm.yaml";
  ...
  std::string vel_topic = config["subscribers"]["velocity_topic"].as<std::string>();
  ...
  ROS_INFO("Subscribing to velocity channel...");
  auto qvel_and_mutex = ros_sub.AddVelocitySubscriber(vel_topic);
  auto qvel = qvel_and_mutex.first;
  auto qvel_mutex = qvel_and_mutex.second;

  ROS_INFO("Velocity data: %s", vel_topic.c_str());
  ...
  InekfEstimator inekf_estimator(
      error_type, project_dir + "/config/tensegrity/inekf_estimator.yaml");
  ...
  inekf_estimator.add_velocity_correction(
      qvel, qvel_mutex,
      project_dir + "/config/tensegrity/velocity_correction.yaml");
