...
/**
 *  @file   wamv_gtodom_vel.cpp
 *  @author Surya Singh
 *  @brief  Test file for WAMV robot setup (IMU Propagation +
 *  Ground-truth based Velocity Correction)
 *  @date   Oct 11, 2024
 **/
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "wamv_gtodom_vel");
  ...
  std::string ros_config_file
      = project_dir + "/ROS/drift/config/wamv_gtodom_vel/ros_comm.yaml";
  ...
  InekfEstimator inekf_estimator(
      error_type, project_dir + "/config/wamv_gtodom_vel/inekf_estimator.yaml");
  ...
  inekf_estimator.add_imu_propagation(
      qimu, qimu_mutex,
      project_dir + "/config/wamv_gtodom_vel/imu_propagation.yaml");
  inekf_estimator.add_velocity_correction(
      qv, qv_mutex,
      project_dir + "/config/wamv_gtodom_vel/velocity_correction.yaml");
