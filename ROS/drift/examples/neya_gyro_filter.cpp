...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "neya_gyro_filter");
  ...
  std::string ros_config_file
      = project_dir + "/ROS/drift/config/neya_gyro_filter/ros_comm.yaml";
  ...
  std::string gyro_topic
      = config["subscribers"]["gyro_topic"].as<std::string>();
  ...
  auto [qgyro, qgyro_mutex] = ros_sub.AddIMUSubscriber(gyro_topic);
  ...
  auto qv_and_mutex
      = ros_sub.AddVelocityWithCovarianceSubscriber(velocity_topic);
  ...
  imu_filter::ImuAngVelEKF gyro_filter(
      project_dir + "/config/neya_gyro_filter/imu_filter.yaml");

  gyro_filter.add_gyro_propagate(qgyro, qgyro_mutex);
  ...
  InekfEstimator inekf_estimator(
      error_type,
      project_dir + "/config/neya_gyro_filter/inekf_estimator.yaml");
  ...
  inekf_estimator.add_imu_propagation(
      ...
      project_dir + "/config/neya_gyro_filter/imu_propagation.yaml");
  inekf_estimator.add_velocity_correction(
      qv, qv_mutex,
      project_dir + "/config/neya_gyro_filter/velocity_correction.yaml");
